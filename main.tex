...
Adding parts per billion to parts per thousand of "dopants" to pure silicon can drastically alter its resistivity, changing it by as much as 8 orders of magnitude! This remarkable property is the cornerstone of modern semiconductor technology. The ability to precisely control the local doping concentration and, consequently, the local electronic properties of a semiconductor crystal is what enables the fabrication of complex semiconductor devices.

Doping levels are typically categorized as follows:

\begin{itemize}
    \item \textbf{N\textsuperscript{--} or P\textsuperscript{--} (Lightly Doped):} The doping concentration lies in the range $10^{14}~\text{cm}^{-3} < N_{D}$ or $N_{A} < 10^{16}~\text{cm}^{-3}$. This level of doping introduces a small number of charge carriers, resulting in relatively high resistivity.
    \item \textbf{N or P (Moderately Doped):} The doping concentration is in the range $10^{16}~\text{cm}^{-3} < N_{D}$ or $N_{A} < 10^{18}~\text{cm}^{-3}$. This is the typical doping level for many semiconductor devices, providing a balance between conductivity and resistivity.
    \item \textbf{N\textsuperscript{+} or P\textsuperscript{+} (Heavily Doped):} The doping concentration is in the range $10^{18}~\text{cm}^{-3} < N_{D}$ or $N_{A} < 10^{20}~\text{cm}^{-3}$. At this level, the material exhibits significantly enhanced conductivity due to the high density of charge carriers.
    \item \textbf{N\textsuperscript{++} or P\textsuperscript{++} (Degenerately Doped):} The doping concentration exceeds $10^{20}~\text{cm}^{-3}$. In this regime, the material behaves almost like a metal, with extremely high conductivity and negligible resistivity.
\end{itemize}
$N_{D}$ and $N_{A}$ represent the donor and acceptor concentrations, respectively.

For reference:
\begin{itemize}
    \item \textbf{Silicon Atomic Density:} The density of silicon atoms in a crystal lattice is approximately $5 \times 10^{22}~\text{cm}^{-3}$.
    \item \textbf{Intrinsic Silicon at Room Temperature:} The intrinsic carrier concentration of silicon at room temperature is $n_{i} = 1.45 \times 10^{10}~\text{cm}^{-3}$. This is the baseline carrier concentration in pure, undoped silicon.
\end{itemize}


\subsection{Dopant Species: P-Type, N-Type}
Dopants are specific atoms introduced into a semiconductor material to modify its electrical properties. These atoms can either donate or accept electrons, thereby creating n-type or p-type regions in the material. For dopants to be electrically active, they must occupy substitutional sites in the crystal lattice. The typical doping levels range from $10^{15}$ to $10^{20}~\text{atoms}/\text{cm}^3$, compared to the atomic density of silicon, which is approximately $5.2 \times 10^{22}~\text{atoms}/\text{cm}^3$.

\begin{itemize}
    \item \textbf{Donor Atoms (n-type):} These atoms donate free electrons to the conduction band, increasing the material's conductivity. Common donor atoms include:
    \begin{itemize}
        \item Phosphorus (P)
        \item Arsenic (As)
        \item Antimony (Sb)
    \end{itemize}
    \item \textbf{Acceptor Atoms (p-type):} These atoms accept electrons, creating holes in the valence band, which act as positive charge carriers. Common acceptor atoms include:
    \begin{itemize}
        \item Boron (B)
        \item Gallium (Ga)
        \item Aluminum (Al)
...
\subsection{Dose and Range Calculations}
...
\subsection{Projected Range and Straggle}
\begin{center}
...
\end{center}
Smart Cut is Soitec's proprietary technology used to manufacture UNIBOND SOI wafers. The process begins with two bulk silicon wafers, one of which is reused to create a SOI wafer later. An oxide layer is grown on one wafer (Wafer A), forming the buried oxide (BOX) layer of the SOI structure. High-dose hydrogen ion implantation through the oxide creates a damage layer at the ion's range.
...
\subsubsection{Etch rate} Depends on the etchant and the material that is been etched; but also:
\begin{itemize}
...
\subsubsection{Selectivity to masking layer(s) and their availability.}When you perform etching, you what to etch ONLY the target material (ideally). Because will also etch the mask and the underlying layer, you must calculate the necessary time to avoid that.
\begin{itemize}
...
\subsubsection{Anisotropy (crystal plane selectivity)} Is a parameter you consider only when etching a monocrystalline material.
...
\subsubsection{Control of etch parameters (according for example to
temperature, agitation conditions,...)}
\subsubsection{Safety of reactant(s) and product(s)}
\subsubsection{Cost (including disposal and fixed costs)}Disposal is usually the higher cost, since is calculated on volume!
\subsubsection{Capacity for etch-stops (simply time controlled or concentration of the solution, doping level, doping dependant or electrochemical etch-stop)}

\section{Wet etching silicon ("HNA")}
\begin{itemize}
...
\section{Anisotropically wet etching Silicon}
\subsection{Inorganic Alkaline Solutions (KOH)}
...
\section{HYDROFLUORIC ACID ($SiO_{2}$)}
\begin{itemize}
...
\subsection{HF-vapor phase etching}
...
\section{RF plasma-based dry etching}
\begin{itemize}
...
\section{TODO:Types of Dry Etching}
...
\subsection{TODO:Reactive Ion Etching (RIE)}
...
\subsection{TODO:Inductively Coupled Plasma (ICP-RIE) Reactor}
...
\section{TODO:Anisotropy in Dry Etching}
...
\subsection{TODO:ASPECT RATIO}

$$Aspect Ratio =\frac{Height}{Width}$$


\section{Deep reactive ion etching (DRIE)$\rightarrow$Time multiplexed deep etching}
\begin{center}
