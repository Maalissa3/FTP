...
\section{Josephson Junctions: Main Concepts and Phenomenology}
...
The Josephson effect also encompasses a dynamical component that links the time derivative of the phase difference to the voltage $V$ across the junction, which follows from the Schr{\"o}dinger equation and Faraday's law:
\begin{equation}
...
\subsection{Energy in a Josephson Junction}
...
$$
E_{J}=\frac{\Phi_{0}I_{c}}{2\pi}=\frac{\hbar I_{c}}{2e}
$$
...
\subsection{Hamiltonian of a Josephson Junction}
...
\subsection{Different Types of Josephson Junctions}
...
\begin{itemize}
    ...
    $$
    where $T$ is the temperature, $D_{diff}$ is the normal metal diffusion constant, and $k_{B}$ is the Boltzmann's constant.
...
\begin{itemize}
    \item \textbf{Point Contacts and Micro-restrictions:} Another practical way to form a junction is by creating a micro-restriction in a superconducting thin film. For widths of a few times $\xi_{s}$, the microbridge behaves as a Josephson weak-link. A phase diagram exists where the behavior transitions from the Josephson effect (JE) to one-dimensional depairing (1d-dp, generated by phase-slips) or Abrikosov vortex motion (AVM) depending on the dimensions (width $W$, length $L$).
    \item \textbf{Grain Boundaries (GBs):} Josephson coupling can occur at grain boundaries, a property widely used to produce HTS Josephson junctions. GBs can be categorized by the fundamental operations that define them: $001$ tilt, $100$ tilt (tilt of the c-axis), or $100$ twist. In HTS, the $d$-wave order-parameter symmetry generates unconventional features at these interfaces.
\end{itemize}

\section{Other Topics and Applications}

The technology of Josephson junctions is extensive and includes:

\begin{itemize}
    \item \textbf{Materials for Fabrication:} A survey on the materials used for the realization of superconducting junctions.
    \item \textbf{Advanced Junctions:} Revisiting the current-phase relation for $d$-wave HTS and Superconductor-Ferromagnet-Superconductor (SFS) junctions, including the study of $\pi$-JJs and hybrid JJs.
    \item \textbf{I-V Characteristics:} Modeling the current-voltage characteristics, notably through the Resistively Shunted Junction (RSJ) model. 
    \item \textbf{Applications in Metrology and SQUIDs:}
    \begin{itemize}
        \item Voltage Metrology, exploiting the a.c. Josephson effect.
        \item Nano-junctions and nano-SQUIDS (Superconducting Quantum Interference Devices).
    \end{itemize}
    \item \textbf{Quantum Devices (Qubits):} A propaedeutic to quantum devices, focusing on Josephson-junction-based qubits.
    \begin{itemize}
        \item The three basic JJ Qubits: Charge Qubit, Flux Qubit, and Phase Qubit.
        \item The Transmon Qubit and extensions/refinements of the basic designs.
        \item Examples of JJ Qubit realization, control (e.g., with SFQ pulses), and readout.
        \item Rf SQUID Qubit and its application in Quantum Annealers.
    \end{itemize}
    \item \textbf{Intrinsic and Stacks of JJs:} Stacks of Josephson junctions and intrinsic JJs in HTS for the generation of THz radiation.
\end{itemize}
